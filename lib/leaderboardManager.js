...
/**
 * Validates if a participant can be added (no duplicates based on profile ID)
...
export function canAddParticipant(existingParticipants, profileUrl) {
  const newProfileId = extractProfileId(profileUrl);
  if (!newProfileId) {
  ...
  return !existingParticipants.some(participant => {
    const existingProfileId = extractProfileId(participant.profileUrl || '');
