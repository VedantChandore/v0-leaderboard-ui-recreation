...
function calculateRealTimeRanking(participants) {
  ...
  const sortedParticipants = participantsWithCorrectTiers.sort((a, b) => {
    ...
    console.log(`🔄 Comparing ${a.name} (${aBadges} badges, ${aLabs} labs, ${a.tier}) vs ${b.name} (${bBadges} badges, ${bLabs} labs, ${b.tier})`);
    ...
    // Secondary: Labs completed (higher is better)  
    if (bLabs !== aLabs) {
    ...
    // Tertiary: Join date (earlier is better)
  ...
  const rankedParticipants = sortedParticipants.map((participant, index) => {
    const rankingScore = (parseInt(participant.badgesEarned) || 0) * 10 + (parseInt(participant.labsCompleted) || 0);
